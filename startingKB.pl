game("7 wonders duel",2,2,2,30,30,10,2.23,["Strategy"],25,false,false,["Card Game","City Building","Civilization"]).
game("7 wonders",2,7,4,30,30,10,2.34,["Family","Strategy"],50,false,false,["Ancient","Card Game","City Building","Civilization"]).
game("30 seconds", 6, 10, 7, 30, 30, 10, 1.18, ["Party"], 30, false, false, ["Party Game", "Real-time", "Trivia"]).
game("Twilight imperium (third edition)",4,6,6,180,240,14,4.25,["Strategy","Thematic"],130,false,false,["Civilization","Negotiation","Political","Science Fiction","Wargame"]).
game("a feast for odin",1,4,3,30,120,12,3.85,["Strategy"],90,false,false,["Economic","Industry","Medieval","Puzzle"]).
game("a game of thrones: the board game (second edition)",3,6,6,120,240,14,3.68,["Strategy"],50,false,false,["Bluffing","Fantasy","Negotiation","Novel-based","Political","Wargame"]).
game("agricola",1,5,4,30,150,12,3.63,["Strategy"],50,false,false,["Animals","Economic","Farming"]).
game("alchemists",2,4,4,120,120,12,3.85,["Strategy"],50,false,false,["Deduction","Fantasy"]).
game("anachrony",1,4,3,30,120,14,3.95,["Strategy"],65,false,false,["Economic","Science Fiction"]).
game("android: netrunner",2,2,2,45,45,12,3.37,["Customizable"],50,false,false,["Bluffing","Card Game","Science Fiction"]).
game("arcadia quest",2,4,4,60,60,10,2.51,["Thematic"],70,false,true,["Adventure","Fantasy"]).
game("arkham horror: the card game",1,2,2,60,120,14,3.28,["Customizable","Thematic"],30,true,true,["Adventure","Card Game","Fantasy","Horror","Novel-Based"]).
game("azul",2,4,2,30,45,8,1.81,["Abstract","Family"],30,false,false,["Renaissance"]).
game("battlestar galactica: the board game",4,6,5,120,300,14,3.23,["Thematic"],150,true,false,["Bluffing","Deduction","Political","Science Fiction","Space Exploration"]).
game("betrayal at house on the hill",3,6,5,60,60,12,2.37,["Thematic"],30,true,false,["Adventure","Exploration","Horror"]).
game("biberclan", 2, 6, 4, 20, 20, 7, 2.00, ["Family"], 11, false, false, ["Card Game"]).
game("black stories", 3, 12, 5, 20, 20, 12, 1.28, ["Party", "Thematic"], 10, false, false,["Card Game", "Deduction", "Horror", "Humor", "Party Game","Murder/Mystery"]).
game("blood rage",3,4,4,60,90,12,2.89,["Strategy","Thematic"],60,false,false,["Fantasy","Mythology"]).
game("bohnanza", 3, 7, 5, 45, 45, 8, 1.67, ["Family"], 7, false, false,["Card Game", "Farming", "Negotiation"]).
game("brass: birmingham",2,4,3,60,120,14,3.88,["Strategy"],60,false,false,["Economic","Industry","Transportation"]).
game("brass: lancashire",2,4,4,60,120,14,3.87,["Strategy"],60,false,false,["Economic","Transportation"]).
game("captain sonar",6,8,8,45,60,10,2.11,["Party","Thematic"],40,false,false,["Deduction","Nautical"]).
game("carcassonne",2,5,2,30,45,8,1.93,["Family"],30,false,false,["City Building","Medieval","Territory Building"]).
game("capo della mafia",3, 7, 4, 30, 30, 10, 1.50, ["Family"], 14, false, false, ["Card Game", "Mafia"]).
game("cards against humanity", 4, 12, 7, 30, 30, 16, 1.22,["Party"], 25, false, false, ["Card Game", "Humor", "Mature"]).
game("castles of mad ludwig",2,4,4,90,90,10,2.66,["Strategy"],40,false,false,["Post-Napoleonic","Puzzle"]).
game("catan",3,4,4,60,120,8,2.34,["Family","Strategy"],45,false,false,["Negotiation"]).
game("caverna: the cave farmers",1,7,4,30,210,12,3.78,["Strategy"],80,false,false,["Animals","Economic","Fantasy","Farming"]).
game("caylus",2,5,3,60,150,12,3.82,["Strategy"],40,false,false,["City Building","Economic","Medieval"]).
game("champions of midgard",2,4,4,60,90,10,2.54,["Strategy"],45,false,false,["Adventure","Dice","Fantasy","Medieval","Mythology"]).
game("chaos in the old world",3,4,4,60,120,14,3.18,["Strategy","Thematic"],60,false,false,["Fantasy","Horror","Mythology","Wargame"]).
game("cheating moth", 4, 5, 5, 30, 30, 6, 1.16, ["Party"], 11, false, false, ["Action", "Card Game"]).
game("claim",2, 2, 2, 25, 25, 8, 1.75, ["Family"], 5, false, false, ["Card Game", "Fantasy"]).
game("clank!: a deck-building adventure",2,4,3,30,60,8,2.21,["Family","Strategy"],40,false,false,["Adventure","Fantasy"]).
game("clans of caledonia",1,4,4,30,120,12,3.43,["Strategy"],50,false,false,["Economic"]).
game("crooks", 2, 4, 4, 15, 15, 10, 1.43, ["Family"],11, false, false, ["Card Game", "Mafia"]).
game("codenames",4,9,6,15,15,10,1.32,["Party"],15,false,false,["Card Game","Deduction","Word Game"]).
game("cockroach salad", 3, 6, 5, 10, 20, 6, 1.15, ["Party"], 10, false, false,["Action", "Card Game", "Humor", "Real-time"]).
game("commands & colors: ancients",2,2,2,60,60,12,2.70,["Wargame"],45,false,false,["Ancient","Wargame"]).
game("concordia",2,5,4,100,100,12,3.09,["Strategy"],50,false,false,["Ancient","Economic","Nautical"]).
game("coup", 3, 6, 5, 15, 15, 10, 1.42, ["Family"], 8, false, false,["Bluffing", "Card Game", "Deduction", "Political"]).
game("coloretto",2, 5, 4, 30, 30, 6, 1.29, ["Family"], 7, false, false, ["Animals", "Card Game"]).
game("dead of winter: a crossroads game",3,5,4,60,120,12,3.00,["Thematic"],50,true,false,["Bluffing","Deduction","Horror"]).
game("deckscape: test time", 1, 4, 2, 30, 90, 10, 1.48, ["Party", "Thematic"], 10, true, false, ["Card Game", "Puzzle", "Real-time"]).
game("descent: journeys in the dark (second edition)",2,5,5,120,120,12,3.21,["Thematic"],80,false,true,["Adventure","Exploration","Fantasy"]).
game("dixit", 4, 6, 6, 30, 30, 8, 1.24, ["Party"], 9, false, false,["Card Game", "Humor", "Party Game"]).
game("dixit: odyssey",4,12,6,30,30,8,1.22,["Party"],40,false,false,["Card Game","Humor"]).
game("dominant species",3,6,4,120,240,14,4.03,["Strategy"],60,false,false,["Animals","Environmental","Prehistoric"]).
game("dominion",2,4,3,30,30,10,2.37,["Strategy"],30,false,false,["Card Game","Medieval"]).
game("dominion: intrigue",2,4,3,30,30,10,2.44,["Strategy"],25,false,false,["Card Game","Medieval"]).
game("drecksau",2, 4, 4, 10, 10, 5, 1.06, ["Family"], 8, false, false, ["Animals", "Card Game"]).
game("eclipse",2,6,4,60,180,12,3.69,["Strategy"],180,false,false,["Civilization","Science Fiction","Space Exploration","Wargame"]).
game("el grande",3,5,5,60,120,12,3.07,["Strategy"],80,false,false,["Renaissance"]).
game("eldritch horror",1,6,4,120,240,14,3.31,["Thematic"],60,true,false,["Adventure","Fantasy","Horror","Novel-based"]).
game("exploding kittens", 3, 5, 4, 15, 15, 8, 1.07, ["Party"], 20, false, false, ["Animals", "Card Game", "Humor"]).
game("fields of arle",1,2,2,60,120,12,3.90,["Strategy"],65,false,false,["Economic","Farming","Industry"]).
game("five cucumbers", 3, 6, 5, 25, 25, 8, 1.32, ["Family"], 5, false, false, ["Card Game"]).
game("five tribes",2,4,2,40,80,12,2.89,["Strategy"],40,false,false,["Animals","Arabian","Fantasy","Mythology"]).
game("fluxx",2, 6, 4, 5, 30, 8, 1.39, ["Family", "Party"], 12, false, false, ["Card Game"]).
game("frank's zoo",4, 7, 6, 6, 1.54, ["Family"], 14, false, false, ["Animals", "Card Game"]).
game("food chain magnate",2,5,3,120,240,14,4.19,["Strategy"],120,false,false,["Economic","Industry"]).
game("forbidden stars",2,4,3,120,180,14,3.79,["Strategy"],120,false,false,["Bluffing","Science Fiction","Wargame"]).
game("forbidden island", 1, 4, 4,30, 30, 8, 1.74, ["Family"],  10, true, false, ["Adventure", "Fantasy"]).
game("forbidden desert", 1, 5, 4, 45, 45, 8, 2.04, ["Family"], 20, true, false, ["Adventure", "Fantasy", "Science Fiction"]).
game("game of trains",2, 4, 4, 30, 30, 8, 1.22, ["Family"], 10, false, false, ["Card Game", "Puzzle", "Trains"]).
game("gaia project",1,4,4,60,150,14,4.30,["Strategy"],80,false,false,["Civilization", "Economic", "Science Fiction", "Territory Building"]).
game("ghost blitz", 2,8,5, 20, 20, 5, 1.14, ["Family", "Party"], 12, false, false,["Action", "Card Game", "Real-time"]).
game("gloomhaven",1,4,3,60,120,14,3.77,["Strategy","Thematic"],120,true,true,["Adventure","Fantasy"]).
game("grand austria hotel",2,3,2,60,120,12,3.23,["Strategy"],80, false, false, ["Economic"]).
game("great western trail",2,4,3,75,150,12,3.69,["Strategy"],60,false,false,["Animals"]).
game("gubs",2, 6, 3, 20, 20, 8, 1.39, ["Family"], 7, false, false, ["Card Game", "Fantasy", "Party Game"]).
game("guillotine", 2, 5, 4, 30, 30, 10, 1.27, ["Family"], 11, false, false, ["Card Game", "Humor", "Napoleonic"]).
game("gier", 4, 5, 5, 15, 15, 6, 1.67, ["Card Game"], 15, false, false, ["Card Game"]).
game("halli galli", 3, 6, 4, 10, 10, 6, 1.01, ["Party"], 13, false, false, ["Action", "Cards"]).
game("hanabi", 2, 5, 4, 25, 25, 10, 1.70, ["Family"],11, true, false, ["Card Game", "Deduction", "Memory"]).
game("hive",2, 2, 2, 20, 20, 8, 2.34, ["Abstract", "Family"], 6, false, false, ["Abstract Strategy", "Animals"]).
game("istanbul",2,5,4,40,60,10,2.59,["Strategy"],35,false,false,["Economic"]).
game("iota", 2, 4, 3, 30, 30, 6, 1.92, ["Abstract"], 5,false, false,["Abstract Strategy", "Card Game"]).
game("jambo", 2, 2, 2, 45, 45, 10, 2.07, ["Strategy"], 40, false, false, ["Card Game", "Economic"]).
game("jumbo jet", 2, 6, 4, 240, 240, 9, 1.67, ["Thematic"], 15, false, false, ["Aviation", "Economic", "Travel"]).
game("joking hazard",3, 7, 5, 30, 90, 16, 1.06, ["Party"], 25, false, false,["Card Game", "Humor", "Mature", "Party Game"] ).
game("kemet",3,5,5,90,120,12,2.99,["Strategy"],60,false,false,["Ancient","Mythology"]).
game("keyflower",2,6,4,90,120,12,3.34,["Strategy"],50,false,false,["City Building","Economic","Nautical","Renaissance","Territory Building"]).
game("kingdom death: monster", 1,4,4,60,180,18,4.24,["Thematic"],300,true,true,["Adventure","Fantasy","Horror","Mature","Mythology"]).
game("le havre",1,4,3,30,150,12,3.75,["Strategy"],60,false,false,["City Building","Economic","Industry", "Nautical"]).
game("legendary encounters: an alien deck building game",1,5,3,30,60,14, 2.71,["Thematic"],50,true,false,["Card Game","Horror","Science Fiction"]).
game("legends of andor", 1, 4, 4, 60, 90, 10, 2.76, ["Thematic"], 25, false, false, ["Adventure", "Fantasy", "Fighting", "Puzzle"]).
game("liar's dice", 3, 6, 5, 15, 30, 8, 1.26, ["Family", "Party"], 20, false, false, ["Bluffing", "Dice", "Party Game"]).
game("ligretto", 3, 4, 4, 10, 10, 6, 1.20, ["Family"], 12, false, false, ["Card Game", "Real-time"]).
game("lisboa",1,4,3,60,120,14,4.50,["Strategy"],80,false,false,["City building","Economic","Political"]).
game("love letter",3, 4, 4, 20, 20, 8, 1.20, ["Family"], 9, false, false, ["Card Game", "Deduction", "Renaissance"]).
game("lords of waterdeep",2,5,4,60,120,10,2.48,["Strategy"],30,false,false,["City Building","Fantasy"]).
game("machiavelli", 4, 8, 7, 480, 480, 16, 3.22, ["Strategy"], 40, false, false, ["Bluffing", "Negotiation", "Political", "Renaissance", "Wargame"]).
game("machi koro",2, 4, 4, 30, 30, 8, 1.55, ["Family"], 8, false, false, ["Card Game", "City Building", "Dice"]).
game("mage knight board game",1,3,2,60,240,14,4.26,["Strategy","Thematic"],200,true,false,["Adventure","Exploration","Fantasy"]).
game("magic maze",2, 6, 4, 15, 15, 8, 1.69, ["Family"],15, true, false,["Exploration", "Fantasy", "Maze", "Real-time"]).
game("magic: the gathering",2, 4, 2, 20, 20, 12, 3.17, ["Customizable"], 30, false, false, ["Card Game", "Collectible", "Fantasy", "Fighting"]).
game("mansions of madness: second edition",1,5,4,120,180,12,2.66,["Thematic"],90,true,false,["Adventure","Exploration","Fantasy","Horror"]).
game("mechs vs. minions",2,4,4,60,90,10,2.46,["Strategy","Thematic"],150,true,true,["Fantasy"]).
game("metropolis", 2, 5, 4, 90, 90, 14, 2.24, ["Family", "Strategy"], 30, false, false, ["Negotiation","City Building"]).
game("mimiq",3, 4, 4, 10, 10, 4, 1.00, ["Family"], 11, false, false, ["Children", "Expressions"]).
game("mister X", 2, 6, 4, 60, 60, 10, 2.07, ["Family"], 30, false, false, ["Bluffing", "Deduction"]).
game("mombasa",2,4,4,75,150,12,3.90,["Strategy"],50,false,false,["Economic"]).
game("monopoly",3,6,4,60,180,8,1.67,["Family"],20,false,false,["Economic","Negotiation"]).
game("munchkin", 3, 6, 4, 60, 120, 12, 1.79, ["Thematic"], 20, false, false, ["Humor","Fighting", "Card Game", "Fantasy"]).
game("mysterium",3,7,5,42,42,8,1.92,["Family","Thematic"],45,true,false,["Deduction","Murder/Mystery"]).
game("nations",1,5,3,40,200,14,3.54,["Strategy"],45,false,false,["Card Game","Civilization","Economic"]).
game("noch mal!", 1, 6, 2, 20, 20, 8, 1.21, ["Family"], 10, false, false, ["Abstract Strategy", "Dice"]).
game("ora et labora",1,4,3,60,180,13,3.91,["Strategy"],140,false,false,["City Building","Economic","Industry","Medieval","Religious","Territory Building"]).
game("orleans",2,4,4,90,90,12,3.09,["Strategy"],50, false,false,["Medieval","Religious","Travel"]).
game("pandemic legacy: season 1",2,4,4,60,60,12,2.83,["Strategy","Thematic"],60,true,true,["Medical","Environmental"]).
game("pandemic",1,4,4,45,45,10,2.42,["Family","Strategy"],35,true,false,["Medical"]).
game("pandemic: iberia",1,5,4,45,45,10,2.64,["Strategy"],50,true,false,["Medical"]).
game("pass the pigs",2, 10, 4, 30, 30, 6, 1.06, ["Family", "Party"], 5, false, false, ["Animals", "Dice", "Party Game"]).
game("paku paku",3, 6, 4, 10, 10, 5, 1.00, ["Family"], 13, false, false, ["Dice"]).
game("patchwork",2,2,2,15,30,6,1.69,["Abstract","Family"],25,false,false,["Puzzle"]).
game("party animals", 3, 5, 4, 45, 45, 8, 1.18, ["Party"], 8, false, false, ["Animals", "Card Game", "Humor", "Party Game"]).
game("panic lab",2, 10, 5, 10, 30, 6, 1.38, ["Family", "Party"], 12, false, false, ["Humor", "Puzzle", "Party Game", "Real-time"]).
game("phase 10", 2, 6, 4, 45, 45, 8, 1.31,["Family"], 13, false, false, ["Card Game"]).
game("pickomino", 2, 7, 4, 20, 20, 6, 1.15,["Family"], 11, false, false, ["Animals", "Dice"]).
game("power grid",3,6,4,120,120,12,3.28,["Strategy"],35,false,false,["Economic","Industry"]).
game("puerto rico",2,5,4,90,150,12,3.29,["Strategy"],30,false,false,["City Building","Economic","Farming"]).
game("qwixx", 2, 5, 4, 15, 15, 8, 1.13, ["Family"], 10, false, false, ["Dice"]).
game("qwirkle",2, 4, 4, 45, 45, 6, 1.64, ["Family", "Abstract"], 7, false, false, ["Abstract Strategy"]).
game("qwinto", 2, 6, 4, 15, 15, 8, 1.38, ["Family"], 9, false, false, ["Dice", "Number"]).
game("race for the galaxy",2,4,2,30,60,12,2.98,["Strategy"],25,false,false,["Card Game","Economic","Science Fiction","Space Exploration"]).
game("raiders of the north sea",2,4,3,60,120,12,2.57,["Strategy"],60,false,false,["Medieval","Nautical"]).
game("railways of the world",3,6,5,120,120,10,3.03,["Strategy"],90,false,false,["Trains","Transportation","Video Game Theme"]).
game("rat-a-tat cat", 2, 6, 4, 10, 10, 5, 1.12, ["Family"], 10, false, false, ["Animals", "Card Game", "Memory"]).
game("rising sun",3,6,4,90,120,14,3.24,["Strategy"],70,false,false,["Bluffing","Fantasy","Mythology","Negotiation","Wargame"]).
game("risk",3,6,4,120,120,10,2.10,["Family","Wargame"],30,false,false,["Territory Building","Wargame"]).
game("robinson crusoe: adventures on the cursed island",1,4,3,60,120,12,3.75,["Thematic"],50,true,false,["Adventure","Exploration","Novel-based"]).
game("roll for the galaxy",2,5,3,45,45,12,2.76,["Strategy"],30,false,false,["Civilization","Dice","Economic","Science Fiction", "Space Exploration"]).
game("root",3,4,4,60,90,10,3.43,["Strategy","Wargame"],60,false,false,["Animals","Fanasy","Wargame"]).
game("rummikub",2, 4, 4, 60, 60, 8, 1.77, ["Family"],12, false, false, ["Abstract Strategy", "Card Game", "Number"]).
game("russian railroads",2,4,4,90,120,12,3.41,["Strategy"],115,false,false,["Industry","Trains"]).
game("santorini",2,4,2,20,20,6,1.78,["Abstract","Family"],20,false,false,["Ancient","Mythology"]).
game("saboteur", 5, 10, 7, 30, 30, 8, 1.33, ["Family", "Party"], 10, false, false, ["Bluffing", "Card Game", "Exploration"]).
game("schotten totten", 2, 2, 2, 20, 20, 8, 1.73, ["Family"], 12, false, false,["Card Game", "Deduction"]).
game("scythe",1,5,4,50,115,12,3.37,["Strategy"],70,false,false,["Economic", "Science Fiction", "Territory Building"]).
game("secret hitler",6,10,8,45,45,14,1.71,["Party"],35,false,false,["Bluffing","Card Game","Humor","Political"]).
game("set",1,8,4,30,30,6,1.69,["Abstract","Family"],10,false,false,["Card Game","Puzzle"]).
game("sherlock holmes consulting detective: the thames murders & other cases",1,5,2,60,120,12,2.72,["Thematic"],50,true,false,["Deduction","Murder/Mystery","Novel-based"]).
game("shit happens",4, 8, 4, 20, 60, 18, 1.17, ["Party"], 21, false, false, ["Card Game", "Party Games"]).
game("shut the box", 1, 4, 3, 30, 30, 5, 1.09, ["Family"],20, false, false, ["Dice", "Math"]).
game("skip-bo", 2, 6, 4, 20, 20, 6, 1.24, ["Family"], 20, false, false,["Card Game"]).
game("small world",2,5,4,40,80,8,2.37,["Family","Strategy"],40,false,false,["Fantasy","Territory Building"]).
game("spirit island",1,4,2,90,120,14,3.91,["Strategy","Thematic"],55,true,false,["Environmental","Fantasy","Mythology", "Territory Building"]).
game("star realms",2,2,2,20,20,8,1.96,["Strategy"],15,false,false,["Card Game","Science Fiction"]).
game("star wars: imperial assault",2,5,5,60,120,12,3.29,["Thematic"],80,false,true,["Adventure","Science Fiction","Wargame"]).
game("star wars: rebellion",2,4,2,180,240,12,3.66,["Strategy","Thematic"],90,false,false,["ScienceFiction","Wargame"]).
game("star wars: x-wing miniatures game",2,3,2,30,45,10,2.48,["Customizable","Wargame"],35,false,false,["Bluffing","Science Fiction","Wargame"]).
game("stone age",2,4,4,60,90,10,2.49,["Family","Strategy"],50,false,false,["Dice","Prehistoric"]).
game("stratego",2,2,2,45,45,8,1.86,["Abstract","Family"],25,false,false,["Bluffing","Deduction","Memory","Napoleonic","Wargame"]).
game("struggle for catan",2, 4, 4, 30, 30, 8, 1.67,["Family"],10, false, false, ["Card Game", "City Building"]).
game("splendor",2, 4, 3, 30, 30, 8, 1.82,["Family"], 30, false, false, ["Card Game", "Economic", "Renaissance"]).
game("spot it", 2, 8, 4, 15, 15, 4, 1.03, ["Party"], 12, false, false, ["Card Game", "Real-time"]).
game("suburbia",1,4,3,90,90,10,2.77,["Strategy"],50,false,false,["City Building","Economic"]).
game("sushi go!", 2, 5, 4, 15, 15, 6, 1.16, ["Family"], 7, false, false, ["Card Game"]).
game("take 5!", 3, 6, 5, 60, 60, 8, 1.48, ["Family"],10, false, false, ["Card Game"]).
game("t.i.m.e stories",3,4,4,90,90,14,2.64,["Thematic"],50,true,false,["Adventure","Mature","Puzzle","Science Fiction"]).
game("terra mystica",2,5,4,60,150,12,3.95,["Strategy"],70,false,false,["Civilization", "Economic", "Fantasy", "Territory Building"]).
game("terraforming mars",1,5,3,120,120,12,3.24,["Strategy"],60,false,false,["Economic","Environmental","Science Fiction","Territory Building"]).
game("the castles of burgundy",2,4,2,30,90,12,3.02,["Strategy"],40,false,false,["Dice","Medieval","Territory Building"]).
game("the gallerist",1,4,4,60,150,14,4.28,["Strategy"],100,false,false,["Economic"]).
game("the great dalmuti",4, 8, 7, 60, 60, 8, 1.30, ["Party"], 6, false, false, ["Card Game", "Medieval"]).
game("the mind",2, 4, 4, 15, 15, 8, 1.04, ["Family", "Party"], 11, false, false, ["Card Game", "Number"]).
game("the resistance: avalon",5,10,7,30,30,12,1.78,["Party"],15,false,false,["Bluffing","Card Game","Deduction","Fantasy","Medieval","Negotiation"]).
game("the voyages of marco polo",2,4,4,40,100,12,3.20,["Strategy"],55,false,false,["Dice","Economic","Medieval","Travel"]).
game("the werewolves of miller's hollow",8,18,12,30,30,10,1.32,["Party"],10,false,false,["Bluffing","Deduction","Horror","Murder/Mystery"]).
game("through the ages: a new story of civilization",2,4,3,180,240,14,4.37,["Strategy"],70,false,false,["Card Game","Civilization","Economic"]).
game("through the ages: a story of civilization",2,4,3,240,240,14,4.17,["Strategy"],50,false,false,["Civilization","Economic"]).
game("ticket to ride: europe",2,5,4,30,60,8,1.96,["Family"],45,false,false,["Trains"]).
game("tigris & euphrates",2,4,4,90,90,12,3.53,["Strategy"],60,false,false,["Ancient","Civilization","Territory Building"]).
game("trajan",2,4,4,60,120,12,3.68,["Strategy"],35,false,false,["Ancient","Political"]).
game("tricky dungeon",2, 4, 3, 20, 40, 10, 2.00, ["Family"], 7, false, false, ["Bluffing", "Fantasy"]).
game("triviant pursuit",2,6,4,90,90,12,1.66,["Family","Party"],30,false,false,["Trivia"]).
game("troyes",2,4,3,90,90,12,3.46,["Strategy"],50,false,false,["Dice","Economic","Medieval"]).
game("tooth terror", 2, 3, 3, 20, 20, 8, 1.00, ["Family"], 5, false, false, ["Card Game", "Humor", "Medical", "Party Game"]).
game("twilight imperium (fourth edition)",3,6,6,240,480,14,4.15,["Strategy"],120,false,false,["Civilization","Economic","Negotiation", "Science  Fiction"]).
game("twilight struggle",2,2,2,120,180,14,3.56,["Strategy","Wargame"],60,false,true,["Political","Modern Warfare"]).
game("tzolk'in: the mayan calendar",2,4,4,90,90,12,3.64,["Strategy"],40,false,false,["Ancient","Civilization","Economic","Farming","Mythology"]).
game("unstable unicorns", 2, 6, 5, 30, 45, 10, 1.65, ["Party"], 24, false, false, ["Card Game", "Humor", "Party Game"]).
game("uno",2, 8, 5, 30, 30, 5, 1.13,["Family"],10, false, false, ["Card Game", "Number"]).
game("viticulture essential edition",1,6,4,45,90,12,2.92,["Strategy"],50,false,false,["Economic","Farming"]).
game("war of the ring (first edition)",2,4,2,120,120,14,3.84,["Thematic","Wargame"],75,false,false,["Adventure","Fantasy","Novel-based","Territory Building","Wargame"]).
game("war of the ring (second edition)",2,4,2,150,180,14,4.06,["Thematic","Wargame"],80,false,true,["Adventure","Fantasy","Novel-Based","Territory Building"]).
game("wizard", 3, 6, 4, 45, 45, 10, 1.74, ["Family"], 6, false, false, ["Card Game"]).
game("you're bluffing", 3, 5, 5, 45, 45, 10, 1.62, ["Family"], 10, false, false, ["Animals", "Bluffing", "Card Game", "Humor"]).
game("yokohama",2,4,3,90,90,12,3.32,["Strategy"],35,false,false,["Economic"]).


/*
cooperative komt voor campaign
game(name, min players, max players, rec players, min time, max time, min age, complexity, type, budget, cooperativeTF, campaignTF, Listgenre)
*/
